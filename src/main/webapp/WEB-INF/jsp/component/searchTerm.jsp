...
<div class='search_word_fragment_wrapper'>
	<%-- <div style="display: flex;">
		<div class='search_word_used_condition_label_wrapper'>용어 조회 조건:</div>
		<div class='search_word_used_condition_value_wrapper'
			id="${param.idPrefix}used_condition"></div>
	</div> --%>
	<div style="display: flex; flex-direction: row;">
		<div style="flex: 1;">
			<div style="display: flex;">
				<div class='search_word_condition_label_wrapper'>신청 기간</div>
			...
			<c:choose>
				<c:when test="${param.bListScreen == 'true'}">
					<div style="display: flex;">
						<div class='search_word_condition_label_wrapper'>상태</div>
						<div class='search_word_condition_value_wrapper'>
							<div style='display: flex; justify-content: space-between; align-items: flex-end; max-width: 400px;'>
								<sec:authorize access="hasRole('USER')">
								<div class="checkbox">
									<label> <input type="checkbox"
										id="${param.idPrefix}checkbox_1"> 신청대기
									</label>
								</div>
								</sec:authorize>
								<div class="checkbox">
									<label> <input type="checkbox"
										id="${param.idPrefix}checkbox_2"> 신청등록
									</label>
								</div>
								<div class="checkbox">
									<label> <input type="checkbox"
										id="${param.idPrefix}checkbox_3"> 승인
									</label>
								</div>
								<div class="checkbox">
									<label> <input type="checkbox"
										id="${param.idPrefix}checkbox_4"> 부결
									</label>
								</div>
								<sec:authorize access="hasRole('USER')">
								<div class="checkbox">
									<label> <input type="checkbox"
										id="${param.idPrefix}checkbox_5"> 신청취소
									</label>
								</div>
								</sec:authorize>
							</div>
			...
			<div style="display: flex;">
				<div class='search_word_condition_label_wrapper'>도메인</div>
				<div class='search_word_condition_value_wrapper'>
					<div class="form-group">
						<input type="text" class="form-control" id="${param.idPrefix}domain_content" placeholder="도메인" autocomplete="off" />
					</div>
				</div>
			</div>
		</div>
		<div style="flex: 1;">
			...
			<div style="display: flex;">
				<div class='search_word_condition_label_wrapper'>용어</div>
				<div class='search_word_condition_value_wrapper less_width'>
					<div class="form-group">
						<input type="text" class="form-control" id="${param.idPrefix}query_content" placeholder="용어" autocomplete="off" />
					</div>
				</div>
			</div>
		</div>
		...
		<script type="text/javascript">
            ...
            $(function () {
            	$("#${param.idPrefix}button").click(function () {
              		...
              		const domainContent = $('#${param.idPrefix}domain_content').val();
...
/*            			let sCondition = '';
           			if (domainContent.trim().length>0){ 
						sCondition += '도메인: "' + domainContent + '"';
           			}	
           			if (queryContent.trim().length > 0) {
           				if (sCondition.length > 0) {
                 			sCondition += ', ';
                  		}
           				sCondition += '용어: "' + queryContent + '"';
          			}
          			if (dateStart.trim().length > 0) {
              			if (sCondition.length > 0) {
                 			sCondition += ', ';
          			...
          			if (dateEnd.trim().length > 0) {
           			...
           			<c:choose>
						<c:when test="${param.bListScreen == 'true'}">
		           			termlist_jqGrid_url = 'term/list?' + jQuery.param(oSearchPayload);
						...
						<c:when test="${param.bListScreen == 'false'}">
							termsearch_jqGrid_url = 'term/list_approved?' + jQuery.param(oSearchPayload);
            	...
            	$("#${param.idPrefix}button_reset").click(function () {
            		...
            		$('#${param.idPrefix}domain_content').val('');
            		$(this).blur();
               	});
            });
        </script>